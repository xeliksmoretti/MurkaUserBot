...
if [ ! -n "$BASH" ]; then
  ...
  bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://github.com/xeliksmoretti/MurkaUserBot/blob/master/install.sh) '"$*"
  exit $?
fi
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
  ...
  if [ ! "$(whoami)" = "root" ]; then
    ...
    if command -v sudo >/dev/null; then
      ...
      sudo "$BASH" -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://raw.githubusercontent.com/MurkaUserBot/MurkaUserBot/master/install.sh) '"$*"
      exit $?
    else
      PKGMGR="true"
    fi
